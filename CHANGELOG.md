...
- **版本号对齐**：合入前端基线后保持后端 `2.9.7`、SPA `6.0.4`。
...
- **ZER-535 邮件文件夹拉取模式**：新前端邮件列表内新增显式的收件箱、垃圾邮件与已删除文件夹切换。每次切换分别向后端传递对应 `folder` 参数，并清理旧文件夹的列表和详情状态。
- **文件夹切换竞态**：列表或详情的旧请求完成后，不再覆盖用户刚切换到的新文件夹结果。
- **简洁模式拉取语义**：明确标记“拉取双箱”会同时拉取收件箱与垃圾邮件。
- **双箱结果反馈**：仅在收件箱与垃圾邮件都成功时显示完整成功；部分成功时明确列出成功和失败的文件夹。

### 测试/验证 / Testing & Verification

- 新增文件夹切换组件测试，覆盖显式模式展示、禁用状态与切换事件。
- 新增邮件服务契约测试，覆盖 `inbox`、`junkemail` 和邮件详情的文件夹参数。
- 后端版本升级到 `2.9.7`，SPA 前端版本升级到 `6.0.4`。
